...
def load_dataset(Dataset):
    if Dataset == 'IP':
        ...
        TOTAL_SIZE = 10249
        TRAIN_VALIDATION_SPLIT = 0.03
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * TRAIN_VALIDATION_SPLIT)
        ...
        TOTAL_SIZE = 3248
        TRAIN_VALIDATION_SPLIT = 0.01
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * TRAIN_VALIDATION_SPLIT)
    
    elif Dataset == 'HT':       
        # classes=15  bands=144    # 349*1905*144
        HT = sio.loadmat('./datasets/Houston2013.mat')
        gt_HT = sio.loadmat('./datasets/Houston2013_gt.mat')
        data_hsi = HT['ori']
        gt_hsi = gt_HT['gt']
        TOTAL_SIZE = 17270     
        TRAIN_VALIDATION_SPLIT = 0.03
        ...
        print('There is no {} dataset here, please check!'.format(Dataset))
        exit()

    return data_hsi, gt_hsi, TOTAL_SIZE, TRAIN_SIZE, TRAIN_VALIDATION_SPLIT
...
def generate_iter(TRAIN_SIZE, train_indices, TEST_SIZE, test_indices, TOTAL_SIZE, total_indices, VAL_SIZE,
    ...
    del all_data,train_data,test_data,x_train,x_test_all,x_val,y_val,x_test,y_test,x1_tensor_train,y1_tensor_train,torch_dataset_train
    del x1_tensor_valida,y1_tensor_valida,torch_dataset_valida,x1_tensor_test,y1_tensor_test,torch_dataset_test,all_tensor_data,all_tensor_data_label,torch_dataset_all

    return train_iter, valiada_iter, test_iter, all_iter #, y_test


def generate_png(all_iter, net, gt_hsi, Dataset, device, total_indices,split):
    pred_test = []
    # with torch.no_grad():
    ...
    classification_map(y_re, gt_hsi, 300,
                       path + '/classification_maps/' + Dataset + '_' + net.name + '_' +str(split)+  '.png')

...
def train(net, train_iter, valida_iter, loss, optimizer, device, plt_path,epochs=30, early_stopping=True,
          early_num=20):
    loss_list = [100]
    early_epoch = 0
    ...
    train_acc_list = []
    valida_acc_list = []
    best_valida_acc=0 #### my add
    least_ep=50  #### my add
    less_valida_loss=100
    ...
    for epoch in range(epochs):
        ...
        print('epoch %d, train loss %.6f, train acc %.4f, valida loss %.6f, valida acc %.4f, time %.1f sec'
                % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n, valida_loss, valida_acc, time.time() - time_epoch))

        PATH = "./net_DBA.pt"

        if epoch >= least_ep:
            if valida_acc>best_valida_acc:
                best_valida_acc=valida_acc
                less_valida_loss=valida_loss
                torch.save(net.state_dict(), PATH)
            elif valida_acc==best_valida_acc and valida_loss<=less_valida_loss:
                less_valida_loss=valida_loss
                torch.save(net.state_dict(), PATH)
    ...
    ###d2l.plt.show()
    d2l.plt.savefig(plt_path)
    d2l.plt.close()
    
    print('epoch %d, loss %.4f, train acc %.4f, time %.1f sec'
